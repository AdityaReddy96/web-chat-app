...
describe("Web Chat Application Tests - Image Sending", function () {
  ...
  it("should send an image", async function () {
    ...
    const plusIconButton = await senderDriver.findElement(By.className('flex justify-center items-center w-11 h-11 rounded-full hover:bg-primary hover:text-white'));
    await plusIconButton.click();

    // Step 2: Click on the image upload option
    const imageOption = await senderDriver.findElement(By.className('flex items-center p-2 px-3 gap-3 hover:bg-slate-200 cursor-pointer'));
    ...
    const imagePath = path.resolve(__dirname, "C:\\Users\\adity\\Downloads\\demo.jpeg"); // Adjust path as needed
