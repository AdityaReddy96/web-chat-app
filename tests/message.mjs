...
describe("Web Chat Application Tests", function () {
  ...
  before(async function () {
    ...
    await login(receiverDriver, "virat@gmail.com", "1234");

    // Navigate to chat interfaces
    await senderDriver.get("http://localhost:3000/676644051459027004a965dc"); // Sender chat URL
    await receiverDriver.get("http://localhost:3000/676647c81459027004a96618"); // Receiver chat URL
  ...
  it("should send and receive messages back and forth", async function () {
    ...
    const sendButtonSender = await senderDriver.findElement(By.css('button.text-primary.hover\\:text-secondary'));
    await sendButtonSender.click();

    // Receiver sends a reply
    const receiverReply = 'Hi, I received your message!';
    const messageInputReceiver = await receiverDriver.findElement(By.id('text'));
    await messageInputReceiver.sendKeys(receiverReply);

    const sendButtonReceiver = await receiverDriver.findElement(By.css('button.text-primary.hover\\:text-secondary'));
